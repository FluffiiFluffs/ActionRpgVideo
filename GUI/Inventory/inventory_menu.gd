...
@onready var inventory_tab = %Inventory
@onready var quests_tab = %Quests
...
func open_inventory():
	...
	#for child in focus_target:
		#for c in child.get_children():
				...
				##print(c)
				#c.grab_focus()
	focus_tab_container(last_tab)
	_update_tab_bar_bottom_neighbor(last_tab)
	if quests_tab.visible:
		quests_tab._on_visible_changed()
...
func _on_tab_changed(_index:int)->void:
	if last_tab != _index:
		last_tab = _index
		await get_tree().process_frame
		_update_tab_bar_bottom_neighbor(_index)
	
##Focuses the tab bar when the inventory opens
func focus_tab_container(_last_tab:int)->void:
	var tabs = tab_container
	var tabbar = tabs.get_tab_bar()
	tabs.call_deferred("set","current_tab",_last_tab)
	if tabbar:
		tabbar.focus_mode = Control.FOCUS_ALL
		tabbar.call_deferred("grab_focus")
	call_deferred("_update_tab_bar_bottom_neighbor", last_tab)
	
func _update_tab_bar_bottom_neighbor(_index: int) -> void:
	var tabbar = tab_container.get_tab_bar()
	if tabbar == null:
		return
	await get_tree().process_frame
	...
	var root: Node = null
	if _index == 0:
		root = inventory_grid_container
	elif _index == 1:
		root = quests_tab
	else:
		root = tab_container.get_tab_control(_index)

	var btn = _first_button_under(root)
	if btn:
		tabbar.focus_neighbor_bottom = btn.get_path()
		btn.focus_neighbor_top = tabbar.get_path()
		
func _first_button_under(node: Node) -> Button:
	if node == null:
		return null
	var b_array := node.find_children("", "Button", true, false)  # recursive
	if b_array.is_empty():
		return null
	# Pick top row first, then leftmost
	b_array.sort_custom(func(a, b):
		if a.global_position.y == b.global_position.y:
			return a.global_position.x < b.global_position.x
		return a.global_position.y < b.global_position.y
	)
	return b_array[0] as Button



##Finds first inventory button after instantiation and sets tabbar focus to that button.
func focus_first_inventory_item()->void:
	...
	for child in inventory_grid_container.get_children():
		if child is PanelContainer:
			for c in child.get_children():
				if c is Button:
					grid_children.append(c)
	var tabbar = tab_container.get_tab_bar()
	tabbar.focus_neighbor_bottom = grid_children[0].get_path()
