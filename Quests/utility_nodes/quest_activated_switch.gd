@tool
@icon("res://ASSETS/Dialogue/quest_switch.png")
...
#typing this variable as the enum allows the enum options to be selected in the dropdown!
#default value set...
...
@export var react_to_global_signal : bool
...
func _ready()->void:
	...
	if react_to_global_signal == true:
		GlobalQuestManager.quest_updated.connect(_on_quest_updated)
	check_is_activated()
	pass

func check_is_activated() -> void:
	# Get the saved quest
	var _q : Dictionary = GlobalQuestManager.find_quest( linked_quest )
...
##loops over all children, sets visible to true, sets process mode to inherit[br]
...
func _on_quest_updated(_qdict:Dictionary)->void: #receives quest dictionary, not resource
	check_is_activated()
	pass
